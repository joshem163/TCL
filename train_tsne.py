...
def train(model, loader, optimizer, device, alpha):
    ...
    for batch_graphs, batch_topo, batch_labels in loader:
        ...
        optimizer.zero_grad()
        logits, proj, _,g_emb, t_emb = model(batch_graphs.x, batch_graphs.edge_index, batch_graphs.batch, batch_topo)
        ...
        #con_loss = contrastive_loss(proj, proj)
        # scale = (cls_loss.detach() / (con_loss.detach() + 1e-8))
        # scaled_con = con_loss * scale
        # loss = cls_loss + alpha * scaled_con
        # print("CLS Loss:", cls_loss.item())
        # print("Contrastive Loss:", scaled_con.item())
...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for batch_graphs, batch_topo, batch_labels in loader:
            ...
            logits, _, _,g_emb, t_emb = model(batch_graphs.x, batch_graphs.edge_index, batch_graphs.batch, batch_topo)
...
def collect_embeddings(model, loader, device):
    ...
    for batch_graphs, batch_topo, batch_labels in loader:
        ...
        gin_embeddings.append(g_emb.cpu())
        mlp_embeddings.append(t_emb.cpu())
        final_embeddings.append(proj.cpu())
        all_labels.append(batch_labels.cpu())

    return {
        "gin": torch.cat(gin_embeddings, dim=0),
        "mlp": torch.cat(mlp_embeddings, dim=0),
        "final": torch.cat(final_embeddings, dim=0),
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='bzr')#MUTAG,PROTEINS,BZR,IMDB-BINARY,COX2,IMDB-MULTI,REDDIT-BINARY,REDDIT-MULTI-5K
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help='Fold for which embeddings will be saved'
    )

    parser.add_argument(
        '--embedding_dir',
        type=str,
        default='saved_embeddings'
    )
    ...
    for train_idx, test_idx in kfold.split(topo_tensor):
        ...
        for epoch in range(1, args.epochs):
            train_loss, train_acc = train(model, train_loader, optimizer, device, alpha=args.alpha)
            val_acc, y_true, y_pred = evaluate(model, test_loader, device)
            if fold_no == args.save_fold:
                epoch_embeddings = collect_embeddings(
                    model,
                    test_loader,
                    device
                )
                ...
                embedding_history.append({
                    "epoch": epoch,
                    "gin": epoch_embeddings["gin"],
                    "mlp": epoch_embeddings["mlp"],
                    "final": epoch_embeddings["final"],
                    "labels": epoch_embeddings["labels"]
                })
        ...
        if fold_no == args.save_fold:
            ...
            embedding_file = os.path.join(
                args.embedding_dir,
                f"{args.dataset}_fold_{fold_no}_embeddings.pt"
            )
            ...
            print(
                f"Embeddings saved to: {embedding_file}"
            )
        fold_no += 1
    print(acc_per_fold)
